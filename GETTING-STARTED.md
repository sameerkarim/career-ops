...
```bash
mkdir -p harness/waves
cp harness/run-config.example.yml harness/waves/wave-001.yml
```
...
```bash
node harness/runner.mjs validate --wave harness/waves/wave-001.yml
```
...
node harness/runner.mjs run    --wave harness/waves/wave-001.yml

# 2) (only if you ran K-02 / xlsx tasks) mechanical workbook audit → audits.jsonl
node harness/runner.mjs audit  --wave harness/waves/wave-001.yml

# 3) LLM judge scores everything → scores.jsonl
node harness/runner.mjs judge  --wave harness/waves/wave-001.yml

# 4) Scorecard → results/wave-001/scorecard.md (also printed)
node harness/runner.mjs report --wave harness/waves/wave-001.yml
```
