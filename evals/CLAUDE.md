# evals/ — Consulting LLM Evaluation Framework
...
3. **Task keys are judge-only.** Nothing under `key:` may ever reach a candidate
   ...
   it when editing.
4. **Run `node evals/harness/runner.mjs validate` after editing** any task, weights,
...
8. **Client material stays out of git.** Tasks are fictional by design. Real
   engagement variants live outside the repo or are fully anonymized — same rule as
   the root data contract's user layer.
...
- npm scripts: `npm run evals:run|evals:judge|evals:report -- --wave <file>`.
