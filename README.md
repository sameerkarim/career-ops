...
1. **Pin the wave manifest.** Copy `harness/run-config.example.yml` →
   ...
   across waves.
2. **Generate.** `node harness/runner.mjs run --wave wave-NNN` executes every
   (task × candidate) pair and writes raw responses + traces to
   `results/wave-NNN/responses.jsonl`.
3. **Judge.** `node harness/runner.mjs judge --wave wave-NNN` scores every
...
5. **Reconcile & report.** `node harness/runner.mjs report --wave wave-NNN`
...
3. ✅ v2 (partial): `artifact-builder` scaffold — real .xlsx/.docx/.pptx via
   ...
   per-client private task variants (keep client-derived
   tasks out of git or fully anonymize them).
